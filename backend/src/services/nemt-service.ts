...
export const NEMTService = {
  ...
  verifyMember(args: {
    ...
    const member = MOCK_MEMBERS.find(m =>
      m.memberId.toLowerCase() === args.member_id.toLowerCase()
    );
    ...
    if (!member) {
      if (DEMO_MODE_LENIENT) {
        ...
        return {
          ...
          phone: '(502) 555-0147',
          address: {
            street: getRandomElement(DEMO_STREETS),
            city: 'Louisville',
            state: 'KY',
            zip: '40216'
          },
  ...
  bookRide(args: {
    ...
    let member = MOCK_MEMBERS.find(m =>
      m.memberId.toLowerCase() === args.member_id.toLowerCase()
    );
    ...
    if (!member) {
      if (DEMO_MODE_LENIENT) {
        console.log(`🔐 DEMO MODE: Creating synthetic booking for member ${args.member_id}`);
        // Create synthetic member for demo booking
        member = {
          memberId: args.member_id,
          firstName: 'Demo',
          lastName: 'Member',
          ...
          phone: '(555) 555-0000',
          address: { street: '123 Demo St', city: 'Louisville', state: 'KY', zip: '40216' },
          planType: 'Medicaid',
          eligibilityStatus: 'active',
          assistanceType: 'ambulatory',
          totalRidesAllowed: 20,
    ...
    // After this point, member is guaranteed to be defined (either found or synthetic)
    // Use non-null assertion since we return early if not found and not in demo mode
    const confirmedMember = member!;

    if (confirmedMember.ridesUsed >= confirmedMember.totalRidesAllowed) {
      return { success: false, error: 'No rides remaining for this benefit period' };
    }

    if (confirmedMember.eligibilityStatus !== 'active') {
    ...
    confirmedMember.ridesUsed += args.trip_type === 'round_trip' ? 2 : 1;
    ...
    return {
      ...
      return_trip: args.trip_type === 'round_trip'
        ? (args.return_trip_type === 'scheduled'
            ...
            : 'Call when ready')
        : undefined,
      remaining_rides: confirmedMember.totalRidesAllowed - confirmedMember.ridesUsed
    };
  },
  ...
  getRideStatus(args: { confirmation_number: string }): {
    ...
    if (!ride) {
      if (DEMO_MODE_LENIENT) {
        ...
        const pickupStreet = getRandomElement(DEMO_STREETS);
        const facility = getRandomElement(DEMO_FACILITIES);
        ...
        return {
          ...
          new_details: {
            pickup: {
              ...
              address: getRandomElement(DEMO_STREETS),
              city: 'Louisville',
              state: 'KY',
              zip: '40216'
            },
