...
export class ConversationLogger {
  ...
  private callMetricsService: CallMetricsService;

  constructor(private dbAdapter: DatabaseAdapter) {
    this.callMetricsService = new CallMetricsService(dbAdapter);
  }
  ...
  async logTurn(
    ...
    try {
      ...
      const turnId = await this.dbAdapter.logConversationTurn({
        ...
        content_text: turn.contentText || null,
  ...
  async logFunctionCall(
    ...
    try {
      const functionCallId = await this.dbAdapter.logFunctionCall({
        conversation_id: conversationId,
        call_id: functionCall.callId || null,
  ...
  async endConversation(
    ...
    try {
      ...
      await this.dbAdapter.endConversation(conversationId, {
        ...
        outcome_details: outcome.outcomeDetails ? JSON.stringify(outcome.outcomeDetails) : null
      });

      // AUTO-LOG CALL METRICS
      try {
        console.log('📊 Auto-calculating call metrics...');
        ...
        const metrics = await this.callMetricsService.calculateMetrics(
          conversationId,
          functionCalls,
          turns
        );

        await this.callMetricsService.recordCallMetrics(metrics as any);
        console.log('✅ Call metrics auto-logged successfully');
      } catch (metricsError) {
        console.error('⚠️  Failed to auto-log call metrics (non-critical):', metricsError);
        // Continue execution - metrics logging failure should not block conversation end
      }
