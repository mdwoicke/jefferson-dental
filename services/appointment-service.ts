...
// Valid appointment types as defined in database schema
type ValidAppointmentType = 'exam' | 'cleaning' | 'exam_and_cleaning' | 'emergency';
...
export class AppointmentService {
  ...
  /**
   * Normalize appointment type to valid database value
   * Handles various AI-generated variations
   */
  private normalizeAppointmentType(input: string): ValidAppointmentType {
    const normalized = input.toLowerCase().trim();

    // Direct matches
    if (['exam', 'cleaning', 'exam_and_cleaning', 'emergency'].includes(normalized)) {
      return normalized as ValidAppointmentType;
    }

    // Handle variations for exam_and_cleaning
    if (
      normalized.includes('exam') && normalized.includes('clean') ||
      normalized === 'exam and cleaning' ||
      normalized === 'exam & cleaning' ||
      normalized === 'examandcleaning' ||
      normalized === 'exam_cleaning' ||
      normalized === 'both' ||
      normalized === 'full checkup' ||
      normalized === 'checkup'
    ) {
      console.log(`📋 Normalized appointment_type "${input}" -> "exam_and_cleaning"`);
      return 'exam_and_cleaning';
    }

    // Handle exam variations
    if (
      normalized === 'dental exam' ||
      normalized === 'initial exam' ||
      normalized === 'dental_exam' ||
      normalized === 'examination'
    ) {
      console.log(`📋 Normalized appointment_type "${input}" -> "exam"`);
      return 'exam';
    }

    // Handle cleaning variations
    if (
      normalized === 'dental cleaning' ||
      normalized === 'teeth cleaning' ||
      normalized === 'dental_cleaning'
    ) {
      console.log(`📋 Normalized appointment_type "${input}" -> "cleaning"`);
      return 'cleaning';
    }

    // Handle emergency variations
    if (
      normalized === 'urgent' ||
      normalized === 'urgent care' ||
      normalized === 'dental emergency'
    ) {
      console.log(`📋 Normalized appointment_type "${input}" -> "emergency"`);
      return 'emergency';
    }

    // Default to exam_and_cleaning for initial appointments (most common case)
    console.warn(`⚠️  Unknown appointment_type "${input}", defaulting to "exam_and_cleaning"`);
    return 'exam_and_cleaning';
  }
  ...
  async bookAppointment(args: {
    ...
    try {
      // Normalize appointment type to handle AI variations
      const normalizedType = this.normalizeAppointmentType(args.appointment_type);
      ...
      // Link children to appointment (case-insensitive matching)
      const children = await this.dbAdapter.getChildrenByPatient(this.currentPatientId);
      const matchedChildNames: string[] = [];

      for (const childName of args.child_names) {
        // Case-insensitive match with trimming
        const child = children.find(c =>
          c.name.toLowerCase().trim() === childName.toLowerCase().trim()
        );
        if (child) {
          await this.dbAdapter.linkChildToAppointment(bookingId, child.id);
          matchedChildNames.push(child.name); // Use the actual name from database
          console.log(`✅ Linked child "${child.name}" (ID: ${child.id}) to appointment`);
        } else {
          console.warn(`⚠️  Child "${childName}" not found in patient record. Available: ${children.map(c => c.name).join(', ')}`);
        }
      }
      ...
      await this.dbAdapter.logAudit({
        ...
        new_value: {
          appointment_time: args.appointment_time,
          appointment_type: normalizedType,
          child_names: matchedChildNames.length > 0 ? matchedChildNames : args.child_names
        },
        changed_by: 'ai_agent',
