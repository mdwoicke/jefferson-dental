...
function applyMigrations(db: Database, fromVersion: number): void {
  ...
  // Migration from version 1 to 2: Add missing appointment fields
  if (currentVersion < 2) {
    console.log('🔧 Applying migration: v1 → v2 (Add appointment fields)');
    ...
    try {
      ...
      if (!columns.includes('child_count')) {
        db.exec('ALTER TABLE appointments ADD COLUMN child_count INTEGER DEFAULT 1');
        console.log('  ✅ Added child_count column');
      }

      if (!columns.includes('rescheduled')) {
        db.exec('ALTER TABLE appointments ADD COLUMN rescheduled INTEGER DEFAULT 0');
        console.log('  ✅ Added rescheduled column');
      }

      if (!columns.includes('reschedule_reason')) {
        db.exec('ALTER TABLE appointments ADD COLUMN reschedule_reason TEXT');
        console.log('  ✅ Added reschedule_reason column');
      }
      ...
      db.exec(`INSERT INTO schema_version (version, description) VALUES (2, 'Added child_count, rescheduled, reschedule_reason, and notes columns to appointments table')`);
...
export async function initBrowserDatabase(): Promise<Database> {
  ...
  if (existingData) {
    ...
    applyMigrations(dbInstance, getCurrentSchemaVersion(dbInstance));

    // Save updated database after migrations
    const data = dbInstance.export();
    await saveToIndexedDB(data);
    console.log('💾 Database saved after migrations');
  } else {
    // Create new database
